...
class BinauralLoss(Module):
    def __init__(self, loss_mode="RTF", win_len=400,
                 win_inc=100, fft_len=1024,sr=16000,rtf_weight=0.3,snr_weight=0.7,
                 ild_weight=0.1, ipd_weight=1, avg_mode="freq"):
                #(self, loss_mode="RTF", win_len=400,.shape
        ...
        self.istft_rtf = ISTFT(win_len=25, win_inc=5, fft_len=512)
        self.rtf_weight = rtf_weight
        self.snr_weight = snr_weight
        self.ild_weight = ild_weight
        self.ipd_weight = ipd_weight
        self.avg_mode = avg_mode
        self.stft_spec = torchaudio.transforms.Spectrogram(n_fft=fft_len,hop_length=win_inc,power=None)
        self.istft_spec = torchaudio.transforms.InverseSpectrogram(n_fft=fft_len, hop_length=win_inc)


    def forward(self, model_output, targets):
        target_stft_l = self.stft_spec(targets[:, 0])
        target_stft_r = self.stft_spec(targets[:, 1])

        output_stft_l = self.stft_spec(model_output[:, 0])
        output_stft_r = self.stft_spec(model_output[:, 1])
        ...
        if self.loss_mode == "RTF":
            ...
            target_stft = torch.stack((target_stft_l,target_stft_r),dim=1) # [batch_size, channel, freq, time] stacking the two channels to create a multichannel signal 
            output_stft = torch.stack((output_stft_l,output_stft_r),dim=1)
            psd_target = psd_transform(target_stft)
            psd_output = psd_transform(output_stft)

            rtf_target = FA.rtf_evd(psd_target)
            rtf_output = FA.rtf_evd(psd_output)
            rtf_error = rtf_target - rtf_output
            
            breakpoint()
            rtf_td_target = self.istft_rtf(rtf_target)
            rtf_td_output = self.istft_rtf(rtf_output)
