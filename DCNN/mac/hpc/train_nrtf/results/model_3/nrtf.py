...
### Training function
def train_epoch(network, device, dataloader, loss_fn, optimizer):
...
def test_epoch(network, device, dataloader, loss_fn, optimizer):
    # Set train mode for both the encoder and the decoder
    network.eval()
    test_loss = []
    ...
    for input_batch in dataloader: # with "_" we just ignore the labels (the second element of the dataloader tuple)
        ...
        # Encode data
        output_batch = network(gpu_input_batch)
        # Evaluate loss
...
def NRTF_main():
    ...
    network = NRTF(N_rtf=N_rtf, ref_ch=1)
    params_to_optimise = [
    ...
    for epoch in range(num_epochs):
        train_loss = train_epoch(network,device,train_loader,loss_fn,optim)
        test_loss = test_epoch(network,device,test_loader,loss_fn,optim)
    ...
    # # _, xe = sig.istft(Xe, time_axis=-1, freq_axis=-2)
    # # _, xe_out = sig.istft(Xe_out, time_axis=-1, freq_axis=-2)
    ...
    # sf.write(home + '/mac/hpc/train_nrtf/input.wav', data=xe, samplerate=16000)
    # sf.write(home + '/mac/hpc/train_nrtf/output.wav', data=xe_out, samplerate=16000)
